...
export default function SubtitleTimeline({
    ...
    const handleContextMenu = (e: MouseEvent<HTMLDivElement>) => {
        ...
        const clickedSub = subtitles.find(sub => time >= sub.start && time <= sub.end);

        if (clickedSub && onSubtitlesDelete) {
            setContextMenu({ x: e.clientX, y: e.clientY, time, subtitleId: clickedSub.id });
            return;
        }
    ...
    return (
        <div className="flex flex-col">
            ...
            {contextMenu && (
                ...
                >
                    {contextMenu.subtitleId != null ? (
                        <button
                            className="flex w-full items-center gap-2.5 px-3 py-2 text-xs text-red-400 transition-colors hover:bg-white/10 hover:text-red-300"
                            onClick={() => {
                                const id = contextMenu.subtitleId!;
